...
## A. First, for a set number of years (lets say 30), simulate climate variables for each year. 
...
## B. Then, use the SSD from a mean matrix to pick a set of plant starting sizes: 
...
## C. for 1 to the number of starting plants (i would make this considerably larger than you think you want to have), you then do the following for each year of time:
...
## D. After doing this, you would go back to the number of new plants in each year: for these, do the same approach as with the starting plants, but starting in the year they are born, and then simulate them going forward. I would use NA for the size in the years before they are 'born'.