    ...
    sel.plt <- sz.startPlts[pp]
      ...
      for (yy in 2:(n.yrs-1)) {  #Loop over years
        ...
        ## --
          ...
          ## --
            sel.plt =  realzd.grwth ## ** does this go here? *
    ...
    for (rr in 1:ncol(mx.sdlgYes)) { #Loop over parent plants
        ...
        for (ss in 1:length(sdlg.strtYr)) { 
          ...
          for (yy in startYr:(n.yrs-2)) {     #For each new seedling, loop over years
              ...
              plt.sz <-  sdlg #dfd ** is this supposed to go up and inside ss loop? ** 
...
## --------------------------------------------------------------
  ...
  name <- as.character("_erbr_SimDat40yrNoMiss.srvCor.sdlgCor.")           #Enter name of file
 ...
 rm(list=ls())
  ...
  datComb1 <- read.csv(file=paste("20240927_erbr_SimDat40yrNoMiss.srvCor.sdlgcor.grwthCor.",dd,".csv",sep=""), header = TRUE)
  ## ------------------------------------------------------------------------------------------------
  ...
  date <- str_replace_all(date, "-", "")
  #type <- "Miss"
  name <- as.character("_erbr_SimDat40yrNoMiss.srvCor.sdlgCor.grwthCor.")        #Enter name of file
