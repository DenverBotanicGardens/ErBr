...
setwd("C:/Users/Dan Doak/Desktop/Students/April/eriogonum models/manu fall2024/glmm troubleshooting")
...
filenames = c('20240926_erbr_SimDat20yrHiGrNoMiss.srvCor.sdlgCor.4.4GLM.rds','20240926_erbr_SimDat20yrNoMiss.srvCor.sdlgCor.3.4GLM.rds')
for (dd in c(1,2)) {
  
  ## Read in data
  noMiss <- readRDS(file=paste(dateGLM, "_erbr_", name, "NoMiss.srvCor.sdlgCor.",dd,".4GLM",".rds", sep=""))
  
  nomiss=readRDS(file = filenames[dd])
  ...
  noMiss$logrosnew=log(noMiss$RosNew)
  ...
  maxes = c(1,1,max(noMiss$TempFall),max(noMiss$TempSummer),max(noMiss$TempWinter),max(noMiss$PptFall),max(noMiss$PptSummer),max(noMiss$PptWinter))
  
  #glmm.grwth <- glmer.nb(RosNew1 ~ log(RosNew) + TempFallSc + TempSummerSc +                            TempWinterSc + PptFallSc + PptSummerSc + PptWinterSc + (1|TransectNew), data=noMiss)                           control=glmerControl(optCtrl=list(maxfun=500)))
  ...
  paramsMM.grwthTmp <- as.data.frame(summary(glmm.grwth)$coef_table[,1])/maxes
  
  #seMM.grwthTmp <- as.data.frame(summary(glmm.grwth)$coefficients[1:nrow(summary(glmm.grwth)$coefficients),2])
  seMM.grwthTmp <- as.data.frame(summary(glmm.grwth)$coef_table[,2])/maxes
