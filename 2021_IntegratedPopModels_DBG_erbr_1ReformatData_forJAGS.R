...
## ------------------------------------------------------------------------------------------------
table(erbr$Transect, erbr$Year, erbr$Site)
# Garden Park West with 5 transects 2004-2013, 2016, 2018, 2020
# Garden Park East with 7 transects 2004-2013, 2016, 2018 (only 4 read in) 2020!!
# Cleora is only 2004
...
## ------------------------------------------------------------------------------------------------
table(erbr.1$Transect, erbr.1$Year, erbr.1$Site)
...
erbr.1 <- erbr.1 %>% 
  ...
  mutate(RosNew=sumNA(Rosettes,na.rm=TRUE), InflNew=sumNA(Infl,na.rm=TRUE)) %>% ungroup()
...
## -----------------------------------------------------------------------------------
erbr.2.merged <- merge(erbr.2, clim3seas, by = "Year") # matches mean temps
...
## SAVE FORMATTED DATA ---------------------------------------------------------------
date <- as.character(Sys.Date())          #Enter date to be added to file name
