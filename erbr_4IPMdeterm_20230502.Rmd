...
vrs.param4to13evn <- readRDS("C:/Users/april/Dropbox/CU_Boulder_PhD/DBG_Internship/erbrVRs_noYRE_4to13even_P1k_20231026")
vrs.param4to13odd <- readRDS("C:/Users/april/Dropbox/CU_Boulder_PhD/DBG_Internship/erbrVRs_noYRE_4to13odd_P1k_20231026")
...
vrs.mean4to13odd <- vrs.param4to13odd %>% group_by(SZ) %>% dplyr::summarize(MEAN_GRWTH=mean(GRWTH),MEAN_GRWTHVAR=mean(GRWTH_VAR),
                                           MEAN_SURV=mean(SURV),MEAN_REPROPROB=mean(REPRO_PROB),
                                           MEAN_REPROAMT=mean(REPRO_AMT),MEAN_NEWPLTS=mean(NEW_PLTS))
vrs.mean9to13 <- vrs.param9to13 %>% group_by(SZ) %>% dplyr::summarize(MEAN_GRWTH=mean(GRWTH),MEAN_GRWTHVAR=mean(GRWTH_VAR),
                                           MEAN_SURV=mean(SURV),MEAN_REPROPROB=mean(REPRO_PROB),
                                           MEAN_REPROAMT=mean(REPRO_AMT),MEAN_NEWPLTS=mean(NEW_PLTS))
...
vrs.minPpt4to13.mean <- vrs.minPpt4to13 %>% group_by(SZ) %>% dplyr::summarize(MEAN_GRWTH=mean(GRWTH),MEAN_GRWTHVAR=mean(GRWTH_VAR),
                                                   MEAN_SURV=mean(SURV),MEAN_REPROPROB=mean(REPRO_PROB),
                                                   MEAN_REPROAMT=mean(REPRO_AMT),MEAN_NEWPLTS=mean(NEW_PLTS))
...
vrs.minPpt9to13 <- subset(vrs.param9to13, YEAR==min.ppt)
vrs.minPpt9to13.mean <- vrs.minPpt9to13 %>% group_by(SZ) %>% dplyr::summarize(MEAN_GRWTH=mean(GRWTH),MEAN_GRWTHVAR=mean(GRWTH_VAR),
                                                   MEAN_SURV=mean(SURV),MEAN_REPROPROB=mean(REPRO_PROB),
                                                   MEAN_REPROAMT=mean(REPRO_AMT),MEAN_NEWPLTS=mean(NEW_PLTS))
...
## Plot results 
par(mfrow=c(1,2))                            #Plot in 4 panels
par(mar=c(9.2,5.5,2,3), mgp = c(2.5, 1, 0))  #bottom, left, top, and right
...
lines(vrs.param$SZ[1:bin.num], vrs.minPpt.mean$MEAN_GRWTH, col="orange",lwd=2)
lines(vrs.param$SZ[1:bin.num], vrs.maxPpt.mean$MEAN_GRWTH, col="dodgerblue",lwd=2)
lines(vrs.param4to13$SZ[1:bin.num], vrs.mean4to13$MEAN_GRWTH, col="black",lwd=2, lty="dashed")
lines(vrs.param4to13$SZ[1:bin.num], vrs.minPpt4to13.mean$MEAN_GRWTH, col="orange",lwd=2, lty="dashed")
lines(vrs.param4to13odd$SZ[1:bin.num], vrs.mean4to13odd$MEAN_GRWTH, col="black",lwd=2, lty="dotted")
lines(vrs.param4to13odd$SZ[1:bin.num], vrs.minPpt4to13odd.mean$MEAN_GRWTH, col="orange",lwd=2, lty="dotted")
lines(vrs.param9to13$SZ[1:bin.num], vrs.mean9to13$MEAN_GRWTH, col="black",lwd=2, lty="dotdash")
lines(vrs.param9to13$SZ[1:bin.num], vrs.minPpt9to13.mean$MEAN_GRWTH, col="orange",lwd=2, lty="dotdash")

legend("bottom", c("Full", "2004-2013", "2004-2013 odd", "2009-2013"), col=c("black"),lwd=2,lty=c("solid","dashed","dotted","dotdash"),
       cex=1.1, inset=c(-0.2,-0.72), xpd=TRUE, horiz=FALSE, bty="n",seg.len=1)
...
legend("bottom", c("All", "Wet", "Dry"), col=c("black","dodgerblue","orange"),lwd=2,cex=1.35,
       inset=c(-0.2,-0.52), xpd=TRUE, horiz=TRUE, bty="n",seg.len=1)
