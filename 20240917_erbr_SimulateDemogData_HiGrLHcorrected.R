...
## Modify 'life history' for Fast Growth to get additional simulated datasets
...
## FOR FAST GRWTH WITH HI MAX PLT SZ  ------------------
minsize <- 1
maxsize <- 600  #300  #For Fast Growth LH datasets 
...
# #
  ...
  medParams$grwthvar_RosCoef <- medParams$grwthvar_RosCoef * 0.05
## Save alternative params for this LH
saveRDS(medParams, file="erbrParams_HiGrAltLHcor_20240925.rds")
# #######################################################
...
## Start data set loop 
name <- as.character("SimDat20yrHiGr") # For naming saved files below
n.datset <- 10
...
print(dd)
    ...
    sel.plt <- sz.startPlts[pp]
      ...
      for (yy in 2:(n.yrs-1)) {  #Loop over years
        ...
        ## --
          ...
          #dfd realzd.grwth <- rnorm(n=1, mean=pred.grwth, sd=sqrt(pred.grwthVar))  
          realzd.grwth <- rnbinom(n=1, size= pred.grwthVar, mu=pred.grwth)  #dfd
         #dfd realzd.grwth <- round(realzd.grwth, digits=0)   #Round to nearest integer since unit is rosettes
 ...
 ## Make counter for needed seedlings each year
  mx.survivors=mx.sz
  mx.survivors[is.na(mx.survivors)==FALSE]=1
  n.survivors=rowSums(mx.survivors,na.rm=TRUE)
  n.sdlgs.needed=(200*0.5)-n.survivors  #*0.25
...
clim32yr <- read.csv("erbr_climData3seas32yr_221114.csv", header=TRUE)
 ...
 for (dd in 1:n.datset) {
  ...
  ## LOAD NO-MISSING DATA TO PROCESS WITH THE FOLLOWING FORMATTING CODE
   datComb1 <- read.csv(file=paste("20240925_erbr_SimDat20yrHiGrNoMiss.srvCor.sdlgCor.",dd,".csv",sep=""), header = TRUE)
  ## ------------------------------------------------------------------------------------------------
