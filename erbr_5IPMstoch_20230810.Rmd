...
#pop.sz.all <- pop.sz.all[]
colnames(pop.sz.all) <- c(as.character(2005:2022),"TRANS","PARAM")
pop.sz.all$`2004` <- startNum  #Add observed num at start
pop.sz.all <- pop.sz.all %>% relocate(`2004`, .before = `2005`)


## ** UPDATES FROM HERE ON DOWN FOR NEW FIG *** 
## Sum across transects
#pop.sz.trans <- pop.sz.all %>% group_by(TRANS) %>% summarise(numIndivs=colSums(, na.rm=TRUE)) 

#colSums(pop.sz.all[1:18])

## Calculate mean and SE of predicted pop size
pop.sz.means <- colMeans(pop.sz.all[1:(length(names.yr)+1)])
pop.sz.sds <- matrixStats::colSds(as.matrix(pop.sz.all[1:(length(names.yr)+1)]))
