...
N.startNum <- 61                        #Set starting pop sz; could increase to better reflect pop sz, not just demog plts in transects
...
## -- OPTIONAL ------   Subset to keep only years from study period (e.g. 2004-2021) 
startYr <- "2007"
...
plot(as.numeric(as.character(num.indivs$Year[4:14])), num.indivs$numIndivs[4:14], type='l', ylim=c(-90,390), 
     ylab="Number of individuals",xlab="Year", cex.lab=1.45, cex.axis=1.3, lwd=2)

lines(2007:2022, pop.sz.means.consec[1:length(pop.sz.means.consec)],
      col="#fc8d59",lwd=1.9)
arrows((2007:2022)+0.15,pop.sz.means.consec[1:length(pop.sz.means.consec)], 
       (2007:2022)+0.15,pop.sz.means.consec[1:length(pop.sz.means.consec)]+pop.sz.sds.consec[1:length(pop.sz.means.consec)], 
       length=0.05, angle=90, col="#fc8d59", lwd=0.6)
arrows((2007:2022)+0.15,pop.sz.means.consec[1:length(pop.sz.means.consec)], 
       (2007:2022)+0.15,pop.sz.means.consec[1:length(pop.sz.means.consec)]-pop.sz.sds.consec[1:length(pop.sz.means.consec)], 
       length=0.05, angle=90, col="#fc8d59", lwd=0.6)
...
arrows(2007:2022,pop.sz.means.full[1:16], 
       ...
       length=0.05, angle=90, col="#d73027", lwd=0.6)
arrows(2007:2022,pop.sz.means.full[1:16], 
       2007:2022,pop.sz.means.full[1:16]-pop.sz.sds.full[1:16], 
       length=0.05, angle=90, col="#d73027", lwd=0.6)

legend("bottomleft", c("Observed", "Full dataset", "2004-2013", "2004-2013 even yrs only", "2004-2013 odd yrs only"), col=c("black","#d73027","#fc8d59","#91bfdb","#4575b4"),lwd=2.95,cex=1,
       horiz=FALSE, bty="y",seg.len=1, xpd=TRUE)
