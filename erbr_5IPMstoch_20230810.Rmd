...
## Calculate mean and 95% quantiles of predicted pop size
## Sum across transects and average across params 
...
quantPopSz95.full <- as.data.frame(pop.sz.params.full[,2:ncol(pop.sz.params.full)]) %>% summarise_all(funs(quantile(., probs=0.95)))
quantPopSz95.full <- as.data.frame(t(quantPopSz95.full))
#quantPopSz95.full <- c(0, quantPopSz95.full)
quantPopSz05.full <- as.data.frame(pop.sz.params.full[,2:ncol(pop.sz.params.full)]) %>% summarise_all(funs(list(quantile(., probs=0.05)))) 
#quantPopSz05.full <- c(0, quantPopSz05.full)
quantPopSz95.consec <- as.data.frame(pop.sz.params.consec[,2:ncol(pop.sz.params.consec)]) %>% 
  summarise_all(funs(list(quantile(., probs=0.95)))) 
#quantPopSz95.consec <- c(0, quantPopSz95.consec)
quantPopSz05.consec <- as.data.frame(pop.sz.params.consec[,2:ncol(pop.sz.params.consec)]) %>% 
  summarise_all(funs(quantile(., probs=0.05)))
quantPopSz05.consec <- as.data.frame(t(quantPopSz05.consec))
#quantPopSz05.consec <- c(0, quantPopSz05.consec)
...
arrows((2008:2022),pop.sz.means.consec[2:length(pop.sz.means.consec)], 
       (2008:2022),quantPopSz05.consec[1:15,1], 
