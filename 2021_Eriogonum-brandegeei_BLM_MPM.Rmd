...
BLM_erbr_lag_scaled[BLM_erbr_lag_scaled$Year == 2018 & BLM_erbr_lag_scaled$StageName == "dead",]
```
...
BLM_erbr_lag_scaled[is.na(BLM_erbr_lag_scaled$Stage),]
...
table(BLM_erbr_scaled$StageName.x, BLM_erbr_scaled$StageName.y)
...
# BLM_MPM[BLM_MPM$StageNum == 1,]
# which(is.na(sapply(1:nrow(BLM_MPM[BLM_MPM$StageNum == 1,]),
#                    function(s) (rpois(1,BLM_MPM$StageNum[BLM_MPM$StageNum == 1][s])+1)^2))==TRUE)

# BLM_MPM$fruit <- 0
# BLM_MPM$fruit[BLM_MPM$StageNum == 1] <- sapply(1:nrow(BLM_MPM[BLM_MPM$StageNum == 1,]), 
#                                                function(s) (rpois(1,BLM_MPM$StageNum[BLM_MPM$StageNum == 1][s])+1)^2)
...
BLM_MPM$fruit <- sapply(1:nrow(BLM_MPM), function(rw){
  if(BLM_MPM$StageNum[rw] == 1 && !is.na(BLM_MPM$StageNum[rw])){
    ...
    max(1, rpois(1,20+(10*BLM_MPM$Ppt.summer.x[rw]))) 
    } else {
      0
      }
  })
