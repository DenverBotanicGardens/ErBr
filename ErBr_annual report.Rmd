...
# Discussion
### our demographic monitoring of Erbr spanning 
...
# Lowest precip in 2011? and consistantly high vpdmax across seasons
climate.long %>%
  group_by(Variable) %>%
  mutate(scale_value = scale(Value)) %>%
  filter(Year > 2003) %>%
...
Figure 6a
```{r}
## Why 2011 is just zero but 2020 are NA? 
erbr.summary %>%
  mutate(SumOfBrowsedInfl = case_when(is.na(SumOfBrowsedInfl) ~ 0,
                                      TRUE ~ SumOfBrowsedInfl)) %>%
  group_by(Year, Site) %>%
...
erbr.summary[erbr.summary$Year == 2011,]

fig6a <- erbr.summary %>%
  mutate(SumOfBrowsedInfl = case_when(is.na(SumOfBrowsedInfl) ~ 0,
                                      TRUE ~ SumOfBrowsedInfl)) %>%
  ggplot(  aes(Year,SumOfBrowsedInfl,colour = Site, shape = Site))+
...
fig6b <- erbr.summary %>%
  mutate(SumOfBrowsedInfl = case_when(is.na(SumOfBrowsedInfl) ~ 0,
                                      TRUE ~ SumOfBrowsedInfl),
         InflBrperInfl = case_when(is.na(InflBrperInfl) ~ 0,
                                      TRUE ~ InflBrperInfl)) %>%
ggplot(   aes(Year,InflBrperInfl,colour = Site, shape = Site))+
  stat_summary(fun = mean, position=position_dodge(width=0.25),
               fun.min = function(x) mean(x) - std.error(x),
               fun.max = function(x) mean(x) + std.error(x),
               geom = "errorbar")+
...
pairs(erbr.seasons)
pairs(erbr.seasons[,grepl("tm", names(erbr.seasons))])
...
lm6 <- glmer(SumOfRosettes ~ 1 + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm7 <- glmer(SumOfRosettes ~  vpdmax_fall + vpdmax_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm8 <- glmer(SumOfRosettes ~  vpdmax_fall + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm22 <- glmer(SumOfRosettes ~  vpdmax_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm9 <- glmer(SumOfRosettes ~ vpdmax_summer + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm10 <- glmer(SumOfRosettes ~ vpdmax_summer + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm11 <- glmer(SumOfRosettes ~ tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm12 <- glmer(SumOfRosettes ~  vpdmax_fall * vpdmax_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm13 <- glmer(SumOfRosettes ~  vpdmax_fall * tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm14 <- glmer(SumOfRosettes ~  vpdmax_fall * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm15 <- glmer(SumOfRosettes ~ vpdmax_summer * tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm16 <- glmer(SumOfRosettes ~ vpdmax_summer * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm17 <- glmer(SumOfRosettes ~ tmin_fall * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm18 <- glmer(SumOfRosettes ~  vpdmax_summer + tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm19 <- glmer(SumOfRosettes ~ vpdmax_fall + tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm20 <- glmer(SumOfRosettes ~ vpdmax_fall + vpdmax_summer + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm21 <- glmer(SumOfRosettes ~ vpdmax_fall + vpdmax_summer + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))


lm.list <- list(lm1,lm2,lm3,lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11,
                lm12, lm13, lm14, lm15, lm16, lm17,
                lm18, lm19, lm20, lm21, lm22)
...
plot_summs(lm1, lm18, scale = TRUE, plot.distributions = TRUE)
...
lm6 <- glmer(SumOfInfl ~ 1 + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm7 <- glmer(SumOfInfl ~  vpdmax_fall + vpdmax_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm8 <- glmer(SumOfInfl ~  vpdmax_fall + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm22 <- glmer(SumOfInfl ~  vpdmax_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm9 <- glmer(SumOfInfl ~ vpdmax_summer + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm10 <- glmer(SumOfInfl ~ vpdmax_summer + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm11 <- glmer(SumOfInfl ~ tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm12 <- glmer(SumOfInfl ~  vpdmax_fall * vpdmax_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm13 <- glmer(SumOfInfl ~  vpdmax_fall * tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm14 <- glmer(SumOfInfl ~  vpdmax_fall * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm15 <- glmer(SumOfInfl ~ vpdmax_summer * tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm16 <- glmer(SumOfInfl ~ vpdmax_summer * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm17 <- glmer(SumOfInfl ~ tmin_fall * vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm18 <- glmer(SumOfInfl ~  vpdmax_summer + tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm19 <- glmer(SumOfInfl ~ vpdmax_fall + tmin_fall + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm20 <- glmer(SumOfInfl ~ vpdmax_fall + vpdmax_summer + vpdmax_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm21 <- glmer(SumOfInfl ~ vpdmax_fall + vpdmax_summer + tmin_fall + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
...
lm6 <- glmer(SumOfRosettes ~ 1 + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm7 <- glmer(SumOfRosettes ~ ppt_winter + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm8 <- glmer(SumOfRosettes ~ ppt_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))
lm9 <- glmer(SumOfRosettes ~ tmax_summer + (1|Site), 
              data = erbr.lms, family = poisson(link = "log"))

lm.list <- list(lm6, lm2,lm3,lm4, lm5, lm7, lm8, lm9)
lm.names <- as.character(unlist(lapply(lm.list,formula)))
(lm.results <- aictab(lm.list, modnames=lm.names))
#evidence ratio 
for(i in 2:length(lm.list)){
...
erbr.lms %>%
  filter(!is.na(Site)) %>%
ggplot(  aes(ppt_winter, SumOfRosettes, color = Site)) +
  geom_point()+
  geom_smooth(method = glm,
              method.args = list(family = poisson))+
  theme_bw()
