...
mean(erbr.summary$SumOfRosettes[erbr.summary$Year<2011 & erbr.summary$Site == "Garden Park East"])
mean(erbr.summary$SumOfRosettes[erbr.summary$Year>=2011 & erbr.summary$Site == "Garden Park East"], na.rm = TRUE)
sd(erbr.summary$SumOfRosettes[erbr.summary$Year<2011 & erbr.summary$Site == "Garden Park East"])
sd(erbr.summary$SumOfRosettes[erbr.summary$Year>=2011 & erbr.summary$Site == "Garden Park East"], na.rm = TRUE)


mean(erbr.summary$SumOfRosettes[erbr.summary$Year<2011 & erbr.summary$Site == "Garden Park West"])
mean(erbr.summary$SumOfRosettes[erbr.summary$Year>=2011 & erbr.summary$Site == "Garden Park West"], na.rm = TRUE)

sd(erbr.summary$SumOfRosettes[erbr.summary$Year<2011 & erbr.summary$Site == "Garden Park West"])
sd(erbr.summary$SumOfRosettes[erbr.summary$Year>=2011 & erbr.summary$Site == "Garden Park West"], na.rm = TRUE)
...
erbr.summary[erbr.summary$Year == 2013,]
...
THIS SHOULD ALL BE FROM NOT MERGED YEAR TO YEAR DATA, JUST the preceding months 
Was .x for rosettes and .y for climate so regressing rosettes on future conditions!!!
```{r}
# AICc
# lm1 <- lm(SumOfRosettes.x~Site, erbr.all.t1) # They are so close and similar, wouldn't assume differences between macroplots
# Crap, .x is year t0, .y is year t1. the sumofrosettes should all be .y

# current year can't change the number of rosettes
lm1 <- glm.nb(SumOfRosettes.y~ as.factor(RustYN.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])
summary(lm1)
ggplot(erbr.all.t1, aes(as.factor(RustYN.x), SumOfRosettes.y))+
  geom_boxplot()

#climate in the previous year
# lm2 <- lm(SumOfRosettes.y~ppt.x, erbr.all.t1[erbr.all.t1$Year.y < 2014,])•
# lm3 <- lm(SumOfRosettes.y~tmin.x, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm4 <- lm(SumOfRosettes.y~tmax.x, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm5 <- lm(SumOfRosettes.y~(ppt.x*tmin.x*tmax.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])

#climate in the current year
lm6 <- glm.nb(SumOfRosettes.y~ppt.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
lm7 <- glm.nb(SumOfRosettes.y~tmin.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
lm8 <- glm.nb(SumOfRosettes.y~tmax.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
lm9 <- glm.nb(SumOfRosettes.y~(ppt.y*tmin.y*tmax.y), erbr.all.t1[erbr.all.t1$Year.y < 2014,])

# #climate interaction of previous and current years
# lm10 <- lm(SumOfRosettes.y~ppt.x*ppt.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm11 <- lm(SumOfRosettes.y~tmin.x*tmin.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm12 <- lm(SumOfRosettes.y~tmax.x*tmax.y, erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm13 <- lm(SumOfRosettes.y~(ppt.y*tmin.y*tmax.y*ppt.x*tmin.x*tmax.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])

# All climate interacting with rust
lm14 <- glm.nb(SumOfRosettes.y~ppt.y*as.factor(RustYN.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])
lm15 <- glm.nb(SumOfRosettes.y~tmin.y*as.factor(RustYN.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])
lm16 <- glm.nb(SumOfRosettes.y~tmax.y*as.factor(RustYN.x), erbr.all.t1[erbr.all.t1$Year.y < 2014,])
# lm17 <- lm(SumOfRosettes.y~(ppt.y+tmin.y+tmax.y+as.factor(RustYN.y))^2, erbr.all.t1[erbr.all.t1$Year.y < 2014,])

# Null model
lm18 <- glm.nb(SumOfRosettes.y~1, erbr.all.t1[erbr.all.t1$Year.y < 2014,])

# With rust and higher temperatures, more rosettes
ggplot(erbr.all[erbr.all$Year < 2014,], aes(tmax, SumOfRosettes, colour = as.factor(RustYN)))+
  geom_point()+
  stat_smooth(method="glm", family="binomial", formula=y~log(x),se=TRUE)
...
summary(lm(SumOfRosettes.y~(ppt.y*tmin.y*tmax.y), erbr.all.t1[erbr.all.t1$Year.y < 2014,]))
