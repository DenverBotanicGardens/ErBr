...
jags_erbrdata <- with(dats, list(Site = Site,Year=Year,RosNew=RosNew,InflNew=InflNew,TagNew=TagNew,surv=surv,
                                 ...
                                 TransectNew=TransectNew,InflYesNo=InflYesNo,RowNum=RowNum,TransectNew.num=TransectNew.num,Year.num=Year.num,
                                 Survs = Survs,
                                 ...
                                 numyears = numyears, numtrans = numtrans))
...
lm1_jags <- function(){
  ...
  grwth_Transect_precision ~ dgamma(0.001, 0.001) # 1 / (grwth_Transect_sigma * grwth_Transect_sigma)     # sigma^2 for tau
  ...
  grwth_Year_precision ~ dgamma(0.001, 0.001) # ~ 1 / (grwth_Year_sigma * grwth_Year_sigma)                 # sigma^2 for tau
  # 
  # 
  ## Survival
